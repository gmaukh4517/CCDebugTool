...
NSString *const kCCNetworkObserverEnabledStateChangedNotification = @"kCCNetworkObserverEnabledStateChangedNotification";
static NSString *const kCCNetworkObserverEnabledDefaultsKey = @"com.cc.CCNetworkObserver.enableOnLaunch";

typedef void (^NSURLSessionAsyncCompletion)(id fileURLOrData, NSURLResponse *response, NSError *error);

@interface CCInternalRequestState : NSObject
...
@implementation CCInternalRequestState

@end

@interface CCNetworkObserver (NSURLConnectionHelpers)
...
@interface CCNetworkObserver (NSURLSessionTaskHelpers)
...
@implementation CCNetworkObserver
...
+ (void)setEnabled:(BOOL)enabled
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:enabled forKey:kCCNetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kCCNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled
{
    return [[[NSUserDefaults standardUserDefaults] objectForKey:kCCNetworkObserverEnabledDefaultsKey] boolValue];
}
...
{
    return NSSelectorFromString([NSString stringWithFormat:@"_cc_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)]);
}
...
{
    static CCNetworkObserver *sharedObserver = nil;
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector sniffingBlock:(void (^)(void))sniffingBlock originalImplementationBlock:(void (^)(void))originalImplementationBlock
{
    ...
    // See https://github.com/Flipboard/CC/issues/61 for an example.
    if (!object) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [CCNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [CCNetworkObserver swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [[CCNetworkObserver sharedObserver] connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^swizzleBlock)(NSURLSessionTask *) = ^(NSURLSessionTask *slf) {
            [[CCNetworkObserver sharedObserver] URLSessionTaskWillResume:slf];
            ((void(*)(id, SEL))objc_msgSend)(slf, swizzledSelector);
        };
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^asyncSwizzleBlock)(Class, NSURLRequest *, NSOperationQueue *, NSURLConnectionAsyncCompletion) = ^(Class slf, NSURLRequest *request, NSOperationQueue *queue, NSURLConnectionAsyncCompletion completion) {
            if ([CCNetworkObserver isEnabled]) {
                NSString *requestID = [self nextRequestID];
                [[CCNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:nil];
                NSString *mechanism = [self mechansimFromClassMethod:selector onClass:class];
                [[CCNetworkRecorder defaultRecorder] recordMechanism:mechanism forRequestID:requestID];
                NSURLConnectionAsyncCompletion completionWrapper = ^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                    [[CCNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:response];
                    [[CCNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                    if (connectionError) {
                        [[CCNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:connectionError];
                    } else {
                        [[CCNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                    }
        ...
        [CCNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSData *(^syncSwizzleBlock)(Class, NSURLRequest *, NSURLResponse **, NSError **) = ^NSData *(Class slf, NSURLRequest *request, NSURLResponse **response, NSError **error) {
            ...
            if ([CCNetworkObserver isEnabled]) {
                ...
                [[CCNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:temporaryResponse];
                [[CCNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                if (temporaryError) {
                    [[CCNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:temporaryError];
                } else {
                    [[CCNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                }
                if (error) {
        ...
        [CCNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:syncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([CCNetworkObserver instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionTask *(^asyncDataOrDownloadSwizzleBlock)(Class, id, NSURLSessionAsyncCompletion) = ^NSURLSessionTask *(Class slf, id argument, NSURLSessionAsyncCompletion completion) {
                ...
                if ([CCNetworkObserver isEnabled] && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechansimFromClassMethod:selector onClass:class];
            ...
            [CCNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncDataOrDownloadSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (void)injectIntoNSURLSessionAsyncUploadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            [CCNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncUploadTaskSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (NSURLSessionAsyncCompletion)asyncCompletionWrapperForRequestID:(NSString *)requestID mechanism:(NSString *)mechanism completion:(NSURLSessionAsyncCompletion)completion
{
    NSURLSessionAsyncCompletion completionWrapper = ^(id fileURLOrData, NSURLResponse *response, NSError *error) {
        ...
        [[CCNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
        if (error) {
            [[CCNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:error];
        } else {
            [[CCNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
        }
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionWillSendRequestBlock undefinedBlock = ^NSURLRequest *(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLRequest *request, NSURLResponse *response) {
        [[CCNetworkObserver sharedObserver] connection:connection willSendRequest:request redirectResponse:response delegate:slf];
        return request;
    };
    ...
    [CCNetworkObserver replaceImplementationOfSelector:selector withSelector:swizzledSelector forClass:cls withMethodDescription:methodDescription implementationBlock:implementationBlock undefinedBlock:undefinedBlock];
}
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLResponse *response) {
        [[CCNetworkObserver sharedObserver] connection:connection didReceiveResponse:response delegate:slf];
    };
...
+ (void)injectDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveDataBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSData *data) {
        [[CCNetworkObserver sharedObserver] connection:connection didReceiveData:data delegate:slf];
    };
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFinishLoadingBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [[CCNetworkObserver sharedObserver] connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectDidFailWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFailWithErrorBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSError *error) {
        [[CCNetworkObserver sharedObserver] connection:connection didFailWithError:error delegate:slf];
    };
...
+ (void)injectTaskWillPerformHTTPRedirectionIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionWillPerformHTTPRedirectionBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSHTTPURLResponse *response, NSURLRequest *newRequest, void(^completionHandler)(NSURLRequest *)) {
        [[CCNetworkObserver sharedObserver] URLSession:session task:task willPerformHTTPRedirection:response newRequest:newRequest completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveDataBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSData *data) {
        [[CCNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveData:data delegate:slf];
    };
...
+ (void)injectDataTaskDidBecomeDownloadTaskIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidBecomeDownloadTaskBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLSessionDownloadTask *downloadTask) {
        [[CCNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didBecomeDownloadTask:downloadTask delegate:slf];
    };
...
+ (void)injectTaskDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLSessionDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLResponse *response, void(^completionHandler)(NSURLSessionResponseDisposition disposition)) {
        [[CCNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveResponse:response completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidCompleteWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionTaskDidCompleteWithErrorBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSError *error) {
        [[CCNetworkObserver sharedObserver] URLSession:session task:task didCompleteWithError:error delegate:slf];
    };
...
+ (void)injectDownloadTaskDidFinishDownloadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidFinishDownloadingBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, NSURL *location) {
        ...
        [[CCNetworkObserver sharedObserver] URLSession:session task:task didFinishDownloadingToURL:location data:data delegate:slf];
    };
...
+ (void)injectDownloadTaskDidWriteDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidWriteDataBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
        [[CCNetworkObserver sharedObserver] URLSession:session downloadTask:task didWriteData:bytesWritten totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite delegate:slf];
    };
...
static char const * const kCCRequestIDKey = "kCCRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask
{
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kCCRequestIDKey);
    if (!requestID) {
...
{
    objc_setAssociatedObject(connectionOrTask, kCCRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id)init
{
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.cc.CCNetworkObserver", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (CCInternalRequestState *)requestStateForRequestID:(NSString *)requestID
{
    CCInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [[CCInternalRequestState alloc] init];
...
@implementation CCNetworkObserver (NSURLConnectionHelpers)

- (void)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)response delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        CCInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        requestState.request = request;
        [[CCNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:response];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        CCInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        [[CCNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:requestState.dataAccumulator];
...
@implementation CCNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        [[CCNetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        if (!requestState.dataAccumulator) {
            ...
            [[CCNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:downloadTask.response];

            NSString *requestMechanism = [NSString stringWithFormat:@"NSURLSessionDownloadTask (delegate: %@)", [delegate class]];
            [[CCNetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
        }

        [[CCNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:bytesWritten];
    }];
}
...
- (void)URLSessionTaskWillResume:(NSURLSessionTask *)task
{
    ...
    [self performBlock:^{
        ...
        if (!requestState.request) {
            ...
            [[CCNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:task.currentRequest redirectResponse:nil];
        }
    }];
}
